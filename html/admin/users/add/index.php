...
if (isset($_REQUEST['emailaddr'])) {
  $emailaddr = $_REQUEST['emailaddr'];
} else {
  $emailaddr = '';
}

if ($action == "insert" && $emailaddr <> "") {
  mysqli_query($dbconn,"INSERT INTO users (email) VALUES('" . $_REQUEST['emailaddr'] . "')");
  $newpkid = $dbconn->insert_id;
  header("Location: /admin/users/view/?pkid=" . $newpkid);
}
?>
<!DOCTYPE html>
<html lang="en">
    ...
    <div class="tulsa-body" style="width: 800px">
      <form method="post">
        ...
        <table>
          <tr>
            <td>Email Address</td>
            <td><input type="text" name="emailaddr" size="50">
          </tr>
          <tr>
            <td>&nbsp;</td>
            <td><input type="submit" value="Add User" class="button">
          </tr>
        </table>
      </form>
    </div>
